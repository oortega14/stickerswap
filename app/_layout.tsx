...
function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Hay session pero el profile aún no terminó de bajar — esperar antes de
    // decidir el redirect. Sin esto, el gate ve `user=null` y manda a tabs
    // antes de poder chequear si el username es auto-generado.
    if (!user) return;

    if (isAutoGeneratedUsername(user.username)) {
    ...
    if (inAuth || inOnboardingIntro) router.replace("/(tabs)" as never);
